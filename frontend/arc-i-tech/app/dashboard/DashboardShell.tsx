...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    persistSharedState(ACCESS_REQUESTS_KEY, accessRequests);
  }, [accessRequests]);
  ...
  useEffect(() => {
    const handleStorage = (event: StorageEvent) => {
      if (event.key === ACCESS_REQUESTS_KEY) {
        if (event.newValue) {
          try {
            setAccessRequests(JSON.parse(event.newValue) as DashboardAccessRequest[]);
          } catch {
            /* noop */
          }
        } else {
  ...
  const handleSubmitProductRequests = (event: React.FormEvent) => {
    ...
    setAccessRequests((previous) => {
      ...
      selectedProducts.forEach((key) => {
        ...
        if (!product || !product.requiresApproval) {
          return;
        }

        const existing = next.find((request) => request.product === key);
        if (existing) {
          ...
          <form onSubmit={handleSubmitProductRequests} className="mt-4 grid gap-4 md:grid-cols-3">
            {productCatalog.map((product) => {
              const request = accessRequests.find(
                (entry) => entry.product === product.key,
              );
              ...
              {accessRequests.length > 0 ? (
                <div className="space-y-2 text-xs">
                  ...
                  {accessRequests.map((request) => {
                    const product = productCatalog.find((item) => item.key === request.product);
                    if (!product) return null;
                    const statusBadge =
